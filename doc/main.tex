...
\begin{array}{rl}
  \multicolumn{2}{l}{x : \t{Identifier} \hfill n : \t{Literal} \hfill ch : \t{Channel}} \\
  \multicolumn{2}{l}{
    B : ch  \mapsto \vec{n} \times \vec{n}~~(\text{Buffers})~~~\hfill~~~
  S = \{ \text{R}, \text{B/}ch, \text{T} \}~~(\text{State})} \\[1ex]
  e := & n ~|~ \lambda x.~e ~|~ x ~|~ e ~ e ~|~ e ; e ~| \t{open} e ~| \t{read} e ~| \t{write} e~e ~|~ch~~\hfill~~
...
\begin{center}
\begin{bpt}
  \AxiomC{$B, e~\rightarrow~B', S', e' $}
  \lab{e-ctx}
  \UnaryInfC{$B, E[e]~\rightarrow~B, S', E[e'] $}
\end{bpt} \hspace{1em}
\begin{bpt}
  \AxiomC{}
  \lab{e-app}
  \UnaryInfC{$B, (\lambda x. e)~v~\rightarrow~B, \text{R}, e[x \mapsto v] $}
\end{bpt} \hspace{1em}
\begin{bpt}
  \AxiomC{}
  \lab{e-seq}
  \UnaryInfC{$B, v_1~;~v_2~\rightarrow~B, \text{R}, v_2 $}
\end{bpt} \\[1em]
\begin{bpt}
  \AxiomC{$ch~$ fresh}
  \lab{e-open}
  \UnaryInfC{$B, \t{open} n~\rightarrow~B[ch \mapsto ((),())], \text{R}, ch $}
\end{bpt} \\[1em]
\begin{bpt}
  \AxiomC{$B(ch) = ((n_0,n_1, ...),\vec{n}_o)$}
  \lab{e-read}
  \UnaryInfC{$B, \t{read} ch~\rightarrow~B[ch \mapsto ((n_1,...),\vec{n}_o)], \text{R}, n_0 $}
\end{bpt} \hspace{1em}
\begin{bpt}
  \AxiomC{$B(ch) = ((),\_)$}
  \lab{e-read-b}
  \UnaryInfC{$B, \t{read} ch~\rightarrow~B, \text{B/}ch, \t{read} ch $}
\end{bpt} \\[1em]
\begin{bpt}
  \AxiomC{$B(ch) = (\vec{n}_i,(..., n_k))$}
  \lab{e-write}
  \UnaryInfC{$B, \t{write} ch~n~\rightarrow~B[ch \mapsto (\vec{n}_i,(... n_k, m))], \text{R}, ch $}
\end{bpt} \\[1em]
\begin{bpt}
  \AxiomC{$B, e \rightarrow B', S', e'$}
  \lab{s-run}
  \UnaryInfC{$B, R, e \leadsto B', S', e' $}
\end{bpt} \hspace{1em}
\begin{bpt}
  \AxiomC{$B(ch) = ((),\vec{n}_o)$}
  \AxiomC{$\text{sync}(ch, \vec{n}_o) = \vec{n}_i$}
  \lab{s-sync}
  \BinaryInfC{$B, \text{B/}ch, e \leadsto B[ch \mapsto (\vec{n}_i, ())], R, e $}
\end{bpt}
